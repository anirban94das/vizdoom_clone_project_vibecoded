...
Things that could make the agent learn better or make experimenting easier, in plain terms. Items 1, 2, and 3 are now built; the rest aren't yet.
...
3. **Better default settings for PPO. ✅ Built.** The training library's defaults were designed for robot-simulation tasks, not pixel games. Training now uses the well-known "Atari recipe" instead (`train_common.PPO_HYPERPARAMS`): collect fewer frames per update, learn in bigger batches, do fewer passes over each batch, and slowly lower the learning rate over the course of training. Takes effect automatically the next time any level trains — nothing to turn on. It's called "Atari-style" because that's the benchmark suite (pixel screen → CNN → pick-one-of-a-few-buttons) these settings were originally tuned for in the PPO paper, and this project has that exact same shape.
...
Items 6, 7 and 8 are the ones flagged as most interesting to dig into next. Items 1, 2, and 3 are done.
